...
Nilcoin Core v 2.0 defines the complete cryptographic foundation for the Nilcoin storage‑based consensus network. **This Core spec is KZG‑only**: any lattice/sealed‑replica research is archived separately and **does not apply to mainnet.**

It specifies, in a fully reproducible manner:

1. **Field & transform algebra** over the CRT-friendly primes (`q₁`, `q₂`) used by **Derive**.
2. **Plaintext possession proofs** — **PoUD** using **KZG polynomial commitments** to DU plaintext, and **PoDE** timed window derivations (normative liveness path).
3. **BLS VRF** and BATMAN aggregation for unbiased beacons.
4. **Dial policy** and governance process for safe parameter evolution.
5. **Security rationale** and Known‑Answer Tests for all normative components.
...
Version 2.0 supersedes v 1.0 and v 1.0‑rc series; it MUST be implemented verbatim by main‑network clients targeting activation height ▢`H_ACT`.
...
| ------ | ------------------------------------------ | ------------------------------- |
| `q`    | Prime field modulus                        | CRT (mandatory): q₁=**998 244 353** and q₂=**1 004 535 809**. Effective modulus Q = q₁ × q₂ ≈ 2^60. |
...
| `"DERIVE_SALT_EXP"` | Salt expansion XOF for k‑limbs          | § 3.3      |
...
## § 2 Message→Vector Commitment (Internal Primitives)
