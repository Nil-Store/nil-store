...
func TestRetrievalSession_SettlesBurnAndPayout(t *testing.T) {
	...
	for i := 0; i < int(types.DealBaseReplication); i++ {
		addrBz := make([]byte, 20)
		copy(addrBz, []byte(fmt.Sprintf("retrieval_settle%02d", i)))
		addr, _ := f.addressCodec.BytesToString(addrBz)
		_, err := msgServer.RegisterProvider(f.ctx, &types.MsgRegisterProvider{
			Creator:      addr,
			Capabilities: "General",
	...
	userBz := make([]byte, 20)
	copy(userBz, []byte("retrieval_settle_user"))
	user, _ := f.addressCodec.BytesToString(userBz)
	userAddr, err := sdk.AccAddressFromBech32(user)
	require.NoError(t, err)
	bank.setAccountBalance(userAddr, sdk.NewCoins(sdk.NewInt64Coin(sdk.DefaultBondDenom, 100)))
	...
	settled, err := f.keeper.RetrievalSessions.Get(sdk.UnwrapSDKContext(f.ctx), openRes.SessionId)
	require.NoError(t, err)
...
func TestRetrievalSession_ExpiryRefundsLockedFee(t *testing.T) {
	bank := newTrackingBankKeeper()
	f := initFixtureWithBankKeeper(t, bank)
	msgServer := keeper.NewMsgServerImpl(f.keeper)
	...
	for i := 0; i < int(types.DealBaseReplication); i++ {
		addrBz := make([]byte, 20)
		copy(addrBz, []byte(fmt.Sprintf("retrieval_exp%02d", i)))
		addr, _ := f.addressCodec.BytesToString(addrBz)
		_, err := msgServer.RegisterProvider(f.ctx, &types.MsgRegisterProvider{
			Creator:      addr,
			Capabilities: "General",
	...
	userBz := make([]byte, 20)
	copy(userBz, []byte("retrieval_exp_user"))
	user, _ := f.addressCodec.BytesToString(userBz)
	userAddr, err := sdk.AccAddressFromBech32(user)
	require.NoError(t, err)
	bank.setAccountBalance(userAddr, sdk.NewCoins(sdk.NewInt64Coin(sdk.DefaultBondDenom, 100)))
	...
	expiredCtx := sdk.UnwrapSDKContext(f.ctx).WithBlockHeight(10)
	_, err = msgServer.ConfirmRetrievalSession(expiredCtx, &types.MsgConfirmRetrievalSession{
		Creator:   user,
		SessionId: openRes.SessionId,
	})
	require.Error(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, math.NewInt(98), dealAfter.EscrowBalance)

	sessionAfter, err := f.keeper.RetrievalSessions.Get(expiredCtx, openRes.SessionId)
	require.NoError(t, err)
...
func TestRetrievalSession_OpenRejectsInsufficientEscrow(t *testing.T) {
	bank := newTrackingBankKeeper()
	f := initFixtureWithBankKeeper(t, bank)
	msgServer := keeper.NewMsgServerImpl(f.keeper)
	...
	for i := 0; i < int(types.DealBaseReplication); i++ {
		addrBz := make([]byte, 20)
		copy(addrBz, []byte(fmt.Sprintf("retrieval_low_p%02d", i)))
		addr, _ := f.addressCodec.BytesToString(addrBz)
		_, err := msgServer.RegisterProvider(f.ctx, &types.MsgRegisterProvider{
			Creator:      addr,
			Capabilities: "General",
	...
	p := types.DefaultParams()
	p.BaseRetrievalFee = sdk.NewInt64Coin(sdk.DefaultBondDenom, 5)
	p.RetrievalPricePerBlob = sdk.NewInt64Coin(sdk.DefaultBondDenom, 6)
	require.NoError(t, f.keeper.Params.Set(f.ctx, p))

	userBz := make([]byte, 20)
	copy(userBz, []byte("retrieval_low_user"))
	user, _ := f.addressCodec.BytesToString(userBz)
	userAddr, err := sdk.AccAddressFromBech32(user)
	require.NoError(t, err)

	bank.setAccountBalance(userAddr, sdk.NewCoins(sdk.NewInt64Coin(sdk.DefaultBondDenom, 10)))
	...
	resDeal, err := msgServer.CreateDeal(f.ctx, &types.MsgCreateDeal{
		...
		ServiceHint:         "General",
		MaxMonthlySpend:     math.NewInt(0),
		InitialEscrowAmount: math.NewInt(5),
	})
	require.NoError(t, err)
	...
	dealBefore, err := f.keeper.Deals.Get(sdk.UnwrapSDKContext(f.ctx), resDeal.DealId)
	require.NoError(t, err)

	_, err = msgServer.OpenRetrievalSession(f.ctx, &types.MsgOpenRetrievalSession{
		Creator:        user,
		DealId:         resDeal.DealId,
		Provider:       dealBefore.Providers[0],
		ManifestRoot:   dealBefore.ManifestRoot,
		StartMduIndex:  0,
		StartBlobIndex: 0,
		BlobCount:      1,
		Nonce:          1,
	...
	dealAfter, err := f.keeper.Deals.Get(sdk.UnwrapSDKContext(f.ctx), resDeal.DealId)
	require.NoError(t, err)
	require.Equal(t, dealBefore.EscrowBalance, dealAfter.EscrowBalance)
