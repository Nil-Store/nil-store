...
/* SEC-CO (CRITICAL): β=16383 is too large for SIS security with q≈2³⁰. Baseline profile implicitly sets β=4095. */
...
| `"NIL_SEAL_ITER_INIT"` | Iterated hash init vector for ζ derivation| § 3.4.2    |
| `"NIL_SEAL_ITER_STEP"` | Iterated hash step for ζ derivation     | § 3.4.2    |
...
/* SEC-CO (HIGH): Increased rounds from 10 to 20 for security margin. */
...
```
/* SEC-CO (CRITICAL): Unsound PRP construction. Key material (ζ_p) MUST NOT be concatenated to the message input. Use BLAKE2s key field. */
...
/* SEC-CO (CRITICAL): 32-bit ζ_p enables pre-computation attacks. Use full 256 bits. */
For `p = 0`, define:
...
/* SEC-CO (CRITICAL): MerkleRoot allows O(log N) parallel computation, undermining sequential work. Replace with O(N) IteratedHash. */
...
• `CoverageTargetDays = 365` (metaspec § 6.0c)
...
/* SEC-CO (CRITICAL): Updated A3 parameters to reflect the audited profile required for security. */
|  A3  | Module‑SIS(m,Q,β) with `m=1024, Q=q₁·q₂≈2⁶⁰, β=4095` provides ≥ 2¹²⁸‑bit security. The witness bound (≈8190) is significantly below the Gaussian Heuristic (≈31595). Justification requires a published estimator run (e.g., Albrecht estimator) explicitly accounting for the circulant/module structure. |
...
/* SEC-CO: Updated KAT descriptions to match remediated parameters. */
| `nilhash.toml` | h^{(1)} (q₁), h^{(2)} (q₂ if CRT), digests, and π transcripts (β=4095) |
| `nilseal_prp.toml` | 20‑round Feistel PRP traces (keyed BLAKE2s, no cycle-walk)            |
...
- Nilhash binding tightened: openings now include a bounded blinding vector `r` with an aggregated proof `π` that `||r||_∞ ≤ β`.
- (Audit Remediation: β adjusted to 4095 and limb size to 12-bit for security). Binding reduces to SIS with a short kernel witness (§ 2.2, § 7.3).
...
- Security text: § 7.3 updated — removed “perfect hiding”; (Audit Remediation: removed flawed statistical hiding claims); clarified SIS‑tight binding.
