...
NilStore is a decentralized storage network designed to provide high‑throughput, verifiable data storage with reduced operational overhead. It relies on a plaintext‑first proof stack: **Proof‑of‑Useful‑Data (PoUD)** using **Kate–Zaverucha–Goldberg (KZG)** commitments for content correctness, and **Proof‑of‑Delayed‑Encode (PoDE)** for timed window derivations, coupled with a **BLS‑based VRF** for unbiased per‑epoch randomness. The system architecture covers the NilFS data layer, the Nil‑Mesh routing protocol, a dual‑token economy ($STOR/$BW), and a hybrid L1/L2 settlement design for EVM compatibility and robust governance.
...
NilStore retains strong cryptographic guarantees while reducing proof and verification overhead on standard CPUs. This democratization of access increases network resilience through geographic distribution and enables a more efficient storage marketplace.
...
SP      ↔  L1:  Epoch proofs (PoUD + PoDE); post `poud_root`, `BW_root`
...
#### 3.2.y.5 Integration with epoch proofs
RS‑2D‑Hex affects only NilFS shard layout; epoch proofs remains file‑agnostic and unmodified (§6).
...
Minted $BW is distributed pro-rata to SPs based on the volume of verified retrieval receipts submitted in their epoch proofs proofs.
...
The economic model is enforced cryptographically through **storage & bandwidth verification** (PoUD + PoDE and audited receipts) on the L1 Data Availability (DA) Chain.

### 6.0a  Proof Mode — Plaintext (normative)

Core supports one normative proof mode: **plaintext** — **PoUD** (KZG multi‑open on DU cleartext) + **PoDE** (timed derivations). Governance MAY tune verification dials (§ 9) and escalation behaviors without altering the plaintext‑first mode.
...
2) Timed derivation (PoDE): Let `W = 8 MiB` (governance‑tunable). The SP MUST compute `deriv = Derive(clear_bytes[interval], beacon_salt, row_id, epoch_id, du_id) (Core § 4.0a)` within the proof window restricted to the interval bytes (no cross‑window state). The proof includes `H(deriv)` and the clear bytes needed for recomputation.
...
*   **Receipt Schema (Normative):**
    ...
    - **Verification model:** Ed25519 signatures are verified **off‑chain by watchers and/or on the DA chain**; epoch proofs only commits to a **Poseidon Merkle root** of receipts and proves byte‑sum consistency. In‑circuit Ed25519 verification is **not required**.
...
2. **PoDE (timed derivation):** For each challenged **W = 8 MiB** window, compute a salted local transform `Derive(clear_window, beacon_salt, row_id, epoch_id, du_id)` **within the proof window** and submit `H(deriv)` with the minimal bytes to recompute. **`R ≥ 16`** sub‑challenges/window and **Σ verified bytes ≥ B_min = 128 MiB** per epoch (defaults; DAO‑tunable).
...
#### 6.3.0 Objective
Strengthen retrieval QoS without suspending reads by sampling and verifying a governance‑tunable fraction of receipts each epoch. This mechanism is additive to epoch proofs and does not alter the proof object.
...
*   **Consensus Parameters (Normative):**
    ...
    *   **Proof Window (`Δ_submit`)**: 1,800 s (30 min) after epoch end — this is the *network scheduling window* for accepting epoch proofs proofs.
...
*   **Slashing Rule (Normative):** Missed **PoUD** proofs (plaintext mode) or missed **epoch proofs** proofs (scaffold mode) trigger a quadratic penalty on the bonded $STOR collateral:
...
It also controls content‑binding dials across Core and Metaspec: epoch proofs PoDE fraction `p_derive` (Core § 4.2.2), and receipt‑level content‑check fraction `p_kzg` (this § 6.3.4).
...
3.  **Public Testnet-0 (L1 DA Chain):** (2026-01) - epoch proofs, basic economics.
...
| Epoch Proof Size (epoch proofs) | ≤ 1.2 kB (post-recursion)            |
...
| --- | --- | --- | --- |
| **Wash‑retrieval / Self‑dealing** | SP scripts fake clients to farm $BW receipts | Challenge‑nonce + expiry in receipts; watchers or L1 verify Ed25519 off‑chain/on‑chain; epoch proofs only commits to **Poseidon receipt root** and byte‑sum; per‑DU/epoch service caps; /16 down‑weighting | §6.1 (Receipt schema & verification model), §6.2 (BW_root), §5.2.1 (caps) |
